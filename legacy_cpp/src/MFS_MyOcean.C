/* Interfaccia MFS di MyOcean */


class ScalarField3D_MFS : public ScalarField3D {
...
public:
  ...
  ScalarField3D_MFS() : ScalarField3D(){} 
  ...
  size_t NTimes(const char netcdf_file[])
  {
    ...
    //status = nc_inq_dimid(ncid, "TIME", &recid); /* Jacopo ECCO2 270721 */ 
...
void ScalarField3D_MFS::Init(const char netcdf_file[], const char nome_lon[], const char nome_lat[],\
			      const char nome_depth[], const char nomeExtension[])
  {
    ...
    cout << "Initializing file of MFS_MyOcean kind\n\n";
    cout.flush();
	...
	//	cout << "Ocean flag: " <<  Lista.ocean << endl;
...
void ScalarField3D_MFS::Load(const char netcdf_file[], const char nome_var[], size_t T, const char nomeExtension[])
  {
    long int i,j,k, ToT;
    size_t nt;
    double v;
    ...
    if(global.backtraj) ToT = ScalarField3D_MFS::ntime - T - 1;
    else ToT = T;

    //    cout << "checking " << nt << " " << ntime << " " << ScalarField3D_MFS::ntime << endl;

    if(ScalarField3D_MFS::ntime <= ToT)
      {
	cout << ScalarField3D_MFS::ntime << " " << ToT << endl;
	...
	for(i=0; i<nx; i++) 
	  {
	    for(j=0; j<ny; j++) 
	      {
		v = *(f + i + nx*j + nx*ny*k);
		// if(fabs(v) > 1.e6)  (*(data+k))[i][j] = 0.;
    if((isnan(v)) || (fabs(v) > 1.e2)) (*(data+k))[i][j] = 0.;
		else (*(data+k))[i][j] = v;
    // cout << v << " " << (*(data+k))[i][j] << "\n";
	      }
	  }
...
double ScalarField3D_MFS :: Value(double X, double Y, double Z) 
{
  size_t k, k1;
  double v1, v2;
  ...
  if(Z >= z[0] || Z < z[nz - 1] || X <= Lon[0] || X >= Lon[nx-1] || Y<= Lat[0] || Y >= Lat[ny-1]) 
    {
      ...
      //      cout << X << " " << Lon[0] << endl;
      //cout << Y << " " << Lat[0] << endl; LP 18-01-2022, commentato perché riempiva il file di log
...
class ScalarField3D_Mask_MFS : public ScalarField3D_Mask, public ScalarField3D_MFS{

public:

 ScalarField3D_Mask_MFS() : ScalarField3D_Mask(), ScalarField3D_MFS(){} 

  void Init(const char netcdf_file[], const char nome_lon[], const char nome_lat[], \
       const char nome_depth[], const char nomeExtension[])  
  {
    ScalarField3D_MFS::Init(netcdf_file, nome_lon, nome_lat, nome_depth, "NULL");
  }
  ...
  {
    return ScalarField3D_MFS::Value(X, Y, Z);
  }
  ...
  size_t NTimes(const char netcdf_file[])
  {
    return ScalarField3D_MFS::NTimes(netcdf_file);
  }

  MatDoub& Points4Neighbour(bool& isUsable) 
  {
    return ScalarField3D_MFS::Points4Neighbour(isUsable);
  }
  ...
  void Mask(const char netcdf_file[], const char nome_var[])
  {
    ...
    ScalarField3D_MFS::ntime = nt;

    //    cout << "numero tempi = " <<     ScalarField3D_Mask_MFS::ntime << endl;

    cout << "checking in mask" << nt << " " << ScalarField3D_MFS::ntime << endl;
    ...
    count[0]=1; count[1]= ScalarField3D_MFS::nz; count[2]= ny; count[3]= nx;
    stride[0] = stride[1] = stride[2] = stride[3] = 1;     

    double *f = new double[ ScalarField3D_MFS::nz * ny * nx];
    ...
    for(k=0; k<ScalarField3D_MFS::nz; k++)
      { 
	for(i=0; i<nx; i++) 
	  {
	    for(j=0; j<ny; j++) 
  ...
  { 
    ...
    size_t k =  ScalarField3D_MFS::nz -1;

    try{
      while( Value(p.X(), p.Y(), ScalarField3D_MFS::z[k] + e) < global.tt && k > 0 ) k--;  
    }
    catch (int ercode)
      {
	cout << "e' proprio qui in bottom\n\n";
	cout << "z = " << ScalarField3D_MFS::z[k] << endl;
      }

    return ScalarField3D_MFS::z[k];
  }
