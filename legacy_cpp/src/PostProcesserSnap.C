...
void Movie(CoupleLagrangianTracer **P, double *time,size_t NTIME, size_t snap_time, size_t NTOT, const char* nomedir)
{
  ...
  gnuplot << "set cbrange [-1000:0]" << endl;
  ...
  for(size_t i=0; i<nsnap; i++)
    {
      ...
      gnuplot << "plot  \"" << nomefile << "\" u 1:2:3 title \"" << TE  << "\"w d  lc palette,\"\" u 4:5:6 notitle w d  lc palette,'/archivio/palatella/world_10m.txt' w d" << endl;
	  ...
	  if( (*(P[i * snap_time]+k)).part1.Alive() &&  (*(P[i * snap_time]+k)).part2.Alive() )
	    {
	      ...
	      snap << (*(P[i * snap_time]+k)).part1.Age(); 
	      snap << endl;
	    }
	}
      snap.close();
...
int main(int argc, char *argv[])
{
  ...
  if(!argv[1])
    {
      cout << "\n\n# PostProcesser 1.0\n\n " << endl;
      printf("\nUsage %s <nome directory simulazione> <numero particelle> <numero processori> <file parametri> [movie snap] \n\n", argv[0]);
      return 0;
    }
  ...
  for(size_t i=0; i<NTIME; i++)
	...
	{
	  cout << "Memoria insufficiente al tempo : " << i << " / " << NTIME << " "  << ba.what() << '\n';
	  cout.flush();
	  throw int(123);
	}
  ...
  for(size_t i=0; i<np; i++) 
	...
	if(nt%movie_snap == 0)
	  {
	    nt /= movie_snap;
	    //	nt = (long)(time / global.snap / movie_snap);
	    i_part = part + i * Npart;
	    Time[nt] = time;
	    count ++;
  ...
  Movie(P, Time, NTIME, 1, NTOT, argv[1]);


  //  DensityAge(P, 30 , 200, 150, NTOT, NTIME, argv[1], 1);
  // DensityAge(P, 60 , 200, 150, NTOT, NTIME, argv[1], 1);
  // DensityAge(P, 90 , 200, 150, NTOT, NTIME, argv[1], 1);
