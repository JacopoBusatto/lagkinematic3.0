...
public:
  ...
  // Static method to calculate and return the scalar dot product of
  // two vectors
  //
  // Note: The dot product of two vectors tell us things about
  // the angle between the vectors. That is, it tells us if they
  // are pointing in the same direction (i.e. are they parallel?
  // If so, the dot product will be 1), or if they're
  // perpendicular (i.e. at 90 degrees to each other) the dot
  // product will be 0, or if they're pointing in opposite
  // directions then the dot product will be -1.
  //
  ...
  // Non-static method to calculate and return the scalar dot
  // product of this vector and another vector
  //
  ...
  // Static method to calculate and return a vector which is the
  // cross product of two vectors
  //
  // Note: The cross product is simply a vector which is
  // perpendicular to the plane formed by the first two
  // vectors. Think of a desk like the one your laptop or
  // keyboard is sitting on.  If you put one pencil pointing
  // directly away from you, and then another pencil pointing to
  // the right so they form a "L" shape, the vector
  // perpendicular to the plane made by these two pencils points
  // directly upwards.
  //
  // Whether the vector is perpendicularly pointing "up" or
  // "down" depends on the "handedness" of the coordinate system
  // that you're using.
  //
  // Further reading: http://en.wikipedia.org/wiki/Cross_product
  //
  // Usage example: Vec3<double> crossVect =
  // Vec3<double>::crossProduct(vectorA, vectorB);
  ...
  void display(std::ostream & ostrm)
  {
    ostrm << x << "\t  " << y << "\t " << z << "\t ";
  }
  ...
  T operator*(const Vec3 &vector) const
  {
  ...
  Vec3 operator^(const Vec3 &vec2) const
  {
    ...
    out.set(y * vec2.z - z * vec2.y, z * vec2.x - x * vec2.z, x * vec2.y - y * vec2.x);
    return out;
  }
  ...
  friend Vec3<T> operator * (T f, Vec3& A)
  {
    return A * f;
  }
